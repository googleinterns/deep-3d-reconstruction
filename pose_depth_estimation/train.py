...
class Trainer:
    def __init__(self, params, output_dir):
        ...
        self.train_dataset = input_fn(tf_records[:30])
        self.val_dataset = input_fn(tf_records[30:])
        num_samples = len(tf_records[:30])
        
        # Optimizer
        # self.total_iteration = (num_samples // params.batch_size) * params.epochs # use if data pipeline has no repeats
        self.total_iteration = num_samples * params.epochs # use if data pipeline repeats
    ...
    def criterions(self, inputs, outputs):
        ...
        for scale in range(self.params.num_scales):
            ...
            norm_disp = tf.image.resize(norm_disp, (h, w))
