...
parser.add_argument("--frame_ids", default=[0, -2, -1, 1, 2], help='frames to load ')
...
class Trainer:
        ...
        with self.strategy.scope():
            for epoch in range(self.start_epoch, self.params.epochs):
                ...
                for i, inputs in enumerate(self.train_dataset):
                    loss, outputs = self.distributed_train_step(inputs)
                    print(
                    ...
                    if i % self.params.draw_every_iter == 0:
                        with self.train_summary_writer.as_default():
                            ...
                            stack_prediction_pred = tf.concat([outputs['pred-20'], outputs['pred-10'], inputs['img'], outputs['pred10'], outputs['pred20']],
                                                              axis=1)
                            stack_prediction_gt = tf.concat([inputs['img-2'], inputs['img-1'], inputs['img'], inputs['img1'], inputs['img2']], axis=1)
                ...
                for i, inputs in enumerate(self.val_dataset):
                    self.distributed_val_step(inputs)
                    print(
    ...
    def distributed_train_step(self, inputs):
        per_replica_losses, per_replica_outputs = self.strategy.run(self.train_step, args=(inputs,))
        return self.strategy.reduce(tf.distribute.ReduceOp.MEAN, per_replica_losses, axis=None), \
                self.strategy.reduce(tf.distribute.ReduceOp.MEAN, per_replica_outputs, axis=None)

       return self.strategy.run(self.val_step, args=(inputs,))
    ...
    def criterions(self, inputs, outputs):
        ...
        for scale in range(self.params.num_scales):
            ...
            norm_disp = tf.image.resize(target_rgb, (h, w))
